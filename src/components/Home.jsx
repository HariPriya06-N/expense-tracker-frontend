...
const Home=()=>{
    ...
    const getExpense=async()=>{
        setLoading(true);
        const res=await axios.get("https://expense-tracker-backend-viuv.onrender.com/get");
        const{data}=res;
        setObj(data);
        setLoading(false);
    ...
    const delete_expense=async(id)=>{
        const res=await axios.delete(`https://expense-tracker-backend-viuv.onrender.com/delete/${id}`);
        getExpense();
    }
    const insertExpense=()=>{
        navigate("/insert");
